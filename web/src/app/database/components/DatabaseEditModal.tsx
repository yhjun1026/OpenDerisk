...
export default function DatabaseEditModal({
  ...
  const dbType = datasource.db_type || datasource.type || '';

  // Backfill form when drawer opens
  useEffect(() => {
    if (!open) return;
    ...
    form.setFieldsValue({
      ...
      description: datasource.comment || datasource.description || '',
    });
  }, [open, dbType, datasource, form]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const [err] = await apiInterceptors(postDbEdit(submitData as any));
      if (err) {
  ...
  return (
    ...
    >
      <Form form={form} layout="vertical" size="middle">
        {/* Database Type Selector — locked in edit mode */}
        <Form.Item
          name="type"
        ...
        >
          <Select
            disabled
            options={[{ label: dbType, value: dbType }]}
