...
import RecallTestModal from "./recall-test-modal";
...
const SyncContent: React.FC<{ name: string; id: number }> = ({ name, id }) => {
  ...
  const handleSync = async (spaceName: string, id: number) => {
    ...
    const res = await apiInterceptors(
      syncDocument(spaceName, { doc_ids: [id] })
    );
...
export default function DocPanel(props: IProps) {
  ...
  const { run: search, loading: searchLoading } = useRequest(
    async (_, doc_name: string) => {
      const [, res] = await apiInterceptors(
        searchDocumentList(space.name, { doc_name })
      );
      return res;
    },
    {
                      ...
                      {searchDocuments.map((document: IDocument) => {
                        return (
                          ...
                          >
                            ...
                            <p>
                              {moment(document.last_sync).format(
                                "YYYY-MM-DD HH:MM:SS"
                              )}
                            </p>
    ...
    <div className="px-4">
      <Space>
        ...
        <Button
          ...
          onClick={() => setRecallTestOpen(true)}
        >
          {t("Recall_test")}
        </Button>
      </Space>
      ...
      <ArgumentsModal
        space={space}
        argumentsShow={argumentsShow}
        setArgumentsShow={setArgumentsShow}
      />
      {/* 编辑弹窗 */}
      <Modal
              ...
              {(fields, { add, remove }) => (
                <>
                  {fields.map(({ key, name }) => (
                    <div
                      key={key}
                      className={cls("flex flex-1 items-center gap-8 mb-6")}
                    >
                      <Form.Item
                        label=""
                        name={[name, "question"]}
