...
function ModelForm({
  ...
  async function getModels() {
    ...
    if (res && res.length) {
      ...
      const grouped = sortedModels.reduce(
        (acc: { [key: string]: SupportModel[] }, model) => {
  ...
  function handleProviderChange(value: string) {
    ...
    const filteredModels = providerModels.filter(
      (m) => m.worker_type === selectedWorkerType
    );
    if (filteredModels.length > 0) {
      const firstModel = filteredModels[0];
      if (firstModel?.params) {
        setParams(
          Array.isArray(firstModel.params)
            ? firstModel.params
            : [firstModel.params]
        );
      }
  ...
  async function onFinish(values: any) {
    ...
    try {
      ...
      const selectedModel = groupedModels[selectedProvider]?.find(
        (m) => m.model === processedValues.name
      );
    ...
    >
      ...
      >
        <Select
          ...
          placeholder={t("model_select_worker_type")}
        >
      ...
      {selectedWorkerType && (
        <FormItem
          label="Provider"
          name="provider"
          rules={[{ required: true, message: t("provider_select_tips") }]}
        >
          <Select
      ...
      {selectedProvider && selectedWorkerType && params && (
        <>
          ...
          <ConfigurableForm
            params={params.filter((p) => p.param_name !== "name")}
            form={form}
          />
        </>
      )}
