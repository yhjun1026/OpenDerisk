...
def _import_pgvector() -> Tuple[Type, Type]:
    from derisk_ext.storage.vector_store.pgvector_store import (
        PGVectorConfig,
        PGVectorStore,
    )
...
def _import_milvus() -> Tuple[Type, Type]:
    from derisk_ext.storage.vector_store.milvus_store import (
        MilvusStore,
        MilvusVectorConfig,
    )
...
def _import_weaviate() -> Tuple[Type, Type]:
    from derisk_ext.storage.vector_store.weaviate_store import (
        WeaviateStore,
...
def _import_oceanbase() -> Tuple[Type, Type]:
    from derisk_ext.storage.vector_store.oceanbase_store import (
        OceanBaseConfig,
        OceanBaseStore,
    )
...
def _import_elastic() -> Tuple[Type, Type]:
    from derisk_ext.storage.vector_store.elastic_store import (
...
def scan_storage_configs():
    ...
    if _HAS_SCAN:
        return
    modules = [
        "derisk_ext.storage.vector_store",
        "derisk_ext.storage.knowledge_graph",
        "derisk_ext.storage.graph_store",
    ]
